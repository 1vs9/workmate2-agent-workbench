...
_ACP_AUTO_APPROVE_ENV = "CLOUDPAW_ACP_AUTO_APPROVE"
...
def _acp_auto_approve_enabled() -> bool:
    """Return True when CloudPaw ACP auto-approve is explicitly enabled."""
    raw = os.environ.get(_ACP_AUTO_APPROVE_ENV)
    if raw is None:
        try:
            from qwenpaw.envs import load_envs

            raw = load_envs().get(_ACP_AUTO_APPROVE_ENV)
        except ImportError:
            raw = None
    if raw is None:
        return False
...
def _pick_allow_option(options: list) -> object | None:
    """Return the least-persistent allow-like option from an ACP options list.

    Prefers one-shot approvals (allow_once / proceed_once) over persistent
    grants (allow_always) so auto-approved tool calls do not widen future
    permission scope.
    """
...
def setup_acp_auto_approve() -> None:
    ...
    Gated by ``CLOUDPAW_ACP_AUTO_APPROVE`` (default off). CloudPaw sets this
    to ``true`` in envs.json during plugin startup so trusted automation keeps
    working, but operators can disable it from the console.
    ...
    3. Otherwise picks the least-persistent allow-like option (``allow_once``
       preferred over ``allow_always``) and returns immediately, without
       suspending the tool for an external respond.

    Non-trusted runners keep the original suspend-and-wait flow intact.
    """
    if not _acp_auto_approve_enabled():
        logger.info(
            "[CloudPaw] ACP auto-approve disabled (%s not enabled); "
            "skipping request_permission patch",
            _ACP_AUTO_APPROVE_ENV,
        )
        return
