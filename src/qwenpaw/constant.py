...
def _read_agentdesk_paths_bootstrap() -> tuple[str, str] | None:
    """Load AgentDesk settings file when env vars are unset."""
    try:
        from .agentdesk.paths_config import load_saved_paths
    ...
    if not saved:
        return None
    working_dir = str(saved.get("working_dir") or "").strip()
    if not working_dir:
        return None
    secret_dir = str(saved.get("secret_dir") or "").strip()
    return working_dir, secret_dir


# WORKING_DIR priority:
# 1. ~/.agentdesk/paths.json (AgentDesk settings UI) → use saved paths
# 2. QWENPAW_WORKING_DIR / COPAW_WORKING_DIR env var is set → use it
# 3. ~/.copaw exists (legacy installation) → use it as-is
# 4. Default → ~/agentdesk
_bootstrap_paths = _read_agentdesk_paths_bootstrap()
_explicit_working_dir = _get_env("QWENPAW_WORKING_DIR")
if _bootstrap_paths is not None:
    WORKING_DIR = Path(_bootstrap_paths[0]).expanduser().resolve()
elif _explicit_working_dir:
    ...
    if _legacy_copaw_dir.exists():
        ...
        _agentdesk_default = Path.home() / "agentdesk"
        WORKING_DIR = _agentdesk_default.resolve()

if _bootstrap_paths is not None:
    _bootstrap_secret = str(_bootstrap_paths[1] or "").strip()
    SECRET_DIR = Path(_bootstrap_secret or f"{WORKING_DIR}.secret").expanduser().resolve()
else:
    SECRET_DIR = (
        Path(
