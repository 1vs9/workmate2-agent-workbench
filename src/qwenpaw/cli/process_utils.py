...
_AGENTDESK_UTILITY_SUBCOMMANDS = (
    "kill",
    "shutdown",
    "init",
    "skills",
...
def _has_agentdesk_utility_subcommand(command: str) -> bool:
    """Return whether the command line is a non-runtime AgentDesk CLI invocation."""
    lowered = f" {command.lower()} "
    return any(f" {name} " in lowered for name in _AGENTDESK_UTILITY_SUBCOMMANDS)
...
def _matches_agentdesk_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a AgentDesk CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m qwenpaw.agentdesk.cli {subcommand}",
            f" qwenpaw.agentdesk.cli {subcommand}",
            f" -m agentdesk {subcommand}",
            f" agentdesk {subcommand}",
            f" agentdesk.exe {subcommand}",
            f'agentdesk.exe" {subcommand}',
        )
    )


def _is_agentdesk_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a AgentDesk CLI wrapper."""
    if _has_agentdesk_utility_subcommand(command):
        return False

    lowered_name = name.lower().removesuffix(".exe")
    if lowered_name == "agentdesk":
        return True

    if _matches_agentdesk_cli_command(command, "app", "desktop"):
        return True

    lowered_command = command.lower()
    return (
        "qwenpaw.agentdesk.cli" in lowered_command
        and " kill " not in f" {lowered_command} "
        and " shutdown " not in f" {lowered_command} "
    )
...
def _is_qwenpaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a QwenPaw/AgentDesk CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    if lowered_name == "qwenpaw":
        return True
    if _is_agentdesk_wrapper_process(name, command):
        return True
    return _matches_qwenpaw_cli_command(
        command,
        "app",
        "desktop",
    )
