...
@click.option(
    "--rebuild-frontend",
    is_flag=True,
    help="Force ``npm run build`` for the AgentDesk UI before starting "
    "(default: only rebuild when web sources changed).",
)
def app_cmd(
    host: str,
    port: int,
    ...
    if is_agentdesk_enabled():
        from ..agentdesk.frontend_build import ensure_frontend_built

        ensure_frontend_built(force=rebuild_frontend)
