...
# Localhost CDP must never be routed through a system/HTTP proxy. A configured
# proxy that does not bypass 127.0.0.1 makes urlopen hang until timeout, which
# previously surfaced as "Timed out waiting for Chrome CDP endpoint".
_NO_PROXY_OPENER = urllib_request.build_opener(
...
async def _wait_for_cdp_ready(
    ...
    while time.monotonic() < deadline:
        # Fail fast if the browser process already exited (e.g. it forwarded
        # the request to an existing instance using the same profile and quit).
        if proc is not None and proc.poll() is not None:
            raise RuntimeError(
                f"Chrome process exited (code {proc.returncode}) before the "
                f"CDP endpoint on port {port} became available. The profile "
                "may be locked by another browser instance.",
            )
        try:
...
async def _start_managed_cdp_browser(
    ...
    pw = None
    try:
        await _wait_for_cdp_ready(chosen_cdp_port, proc=proc)
...
def _clear_profile_singleton_locks(user_data_dir: str) -> None:
    """Free a Chrome profile directory before (re)launching against it.

    Leaked/orphaned Chrome processes that still hold this exact
    ``--user-data-dir`` cause new launches to either time out waiting for the
    CDP endpoint (the new process forwards to the existing instance and exits)
    or fail immediately with "Target page, context or browser has been closed".

    This only targets browser processes whose command line references *this*
    managed profile path, so the user's personal browser is never touched.
    It also removes stale ``Singleton*`` lock files left by crashed instances.
    """
    ...
    # 1) Kill orphaned browser processes still bound to this profile dir.
    try:
        import psutil  # noqa: PLC0415  (lazy import; declared dependency)
        ...
        for proc in psutil.process_iter(["name", "cmdline"]):
            try:
                ...
                if target_norm in joined:
                    proc.kill()
            except Exception:
                continue
    except Exception:
        logger.debug(
            "Could not scan for orphaned browser processes",
            exc_info=True,
        )

    # 2) Remove stale singleton lock files (Linux/macOS; harmless on Windows).
    ...
    try:
        if _USE_SYNC_PLAYWRIGHT:
            ...
            try:
                ...
                # _action_start reports failures via its return value rather
                # than raising, so verify the fallback actually established a
                # live browser. Otherwise the caller would dereference a None
                # context (e.g. "'NoneType' object has no attribute 'new_page'").
                if not _is_browser_running(state):
                    state["_last_browser_error"] = (
                        state.get("_last_browser_error")
                        or f"Browser launch failed (CDP: {cdp_exc})"
                    )
                    return False
    ...
    try:
        if not _USE_SYNC_PLAYWRIGHT and not bool(private_mode):
            ...
            if exe:
                ...
                if user_data_dir:
                    Path(user_data_dir).mkdir(parents=True, exist_ok=True)
                    # Free any leaked/orphaned browser holding this profile so
                    # the persistent context does not close immediately.
                    _clear_profile_singleton_locks(user_data_dir)
