...
from .context.context_helpers import append_messages_to_dialog
...
from .tools import (
    ...
    chat_with_agent,
    check_agent_task,
    create_agentdesk_employee,
    create_agentdesk_employees,
    submit_to_agent,
    desktop_screenshot,
    edit_file,
...
class QwenPawAgent(CodingModeMixin, Agent):
    ...
    async def compress_context(
        ...
        """Compact the context using QwenPaw's own (thinking-safe) compactor.

        agentscope's native ``compress_context`` summarises via
        ``model.generate_structured_output``, which forces a ``tool_choice``.
        Reasoning models reject a forced ``tool_choice`` while *thinking mode*
        is active (e.g. DeepSeek: "Thinking mode does not support this
        tool_choice"), and because that call sits on the reply path with no
        fallback it aborts the whole stream.

        This override keeps agentscope's trigger detection and context split,
        but routes summary generation through
        ``LightContextManager.compact_context`` — a plain-text ``agent.reply``
        that never forces a tool call, so it is thinking-mode safe and matches
        the ``/compact`` command path. Evicted messages are persisted to the
        per-agent dialog log so nothing is lost, and any failure degrades
        gracefully instead of crashing the reply.
        """
        cm = self.context_manager
        if cm is None or not hasattr(cm, "compact_context"):
            # No QwenPaw context manager — keep agentscope's native behaviour.
        ...
        try:
            ...
            estimated_tokens = await self.model.count_tokens(**model_input)
            ...
            if not self.state.context:
                logger.warning(
                    "[AGENT %s]: system prompt and summary alone exceed the "
                    "compression threshold (%d tokens); nothing to compact. "
                    "Use /clear or /new to reset the context.",
                    self.name,
                    int(threshold),
                )
                return
            ...
            logger.info(
                "[AGENT %s]: token count %d exceeds threshold %d, compacting "
                "via QwenPaw compactor.",
                self.name,
            ...
            if not msgs_to_compress:
                # Reserve ratio too large to compress anything; drop it.
                (
                    msgs_to_compress,
                    msgs_to_reserve,
                ) = await self._split_context_for_compression(0, tools)
            if not msgs_to_compress:
                return

            result = await cm.compact_context(
                messages=msgs_to_compress,
                previous_summary=self.state.summary or "",
            )

            # Persist evicted messages off-context before trimming so the
            # full history survives in the per-agent dialog log.
            dialog_path = (
                cm.get_dialog_path()
                if hasattr(cm, "get_dialog_path")
                else None
            )
            await append_messages_to_dialog(msgs_to_compress, dialog_path)

            self.state.context = msgs_to_reserve
            if result.get("success") and result.get("history_compact"):
                self.state.summary = result["history_compact"]
            else:
                logger.warning(
                    "[AGENT %s]: compaction produced no summary (%s); kept "
                    "previous summary and trimmed older context.",
                    self.name,
                    result.get("reason", "unknown"),
                )

            logger.info(
                "[AGENT %s]: QwenPaw context compression finished.",
                self.name,
            )
        except Exception:
            # Compaction must never abort the reply stream.
            logger.exception(
                "[AGENT %s]: context compression failed; continuing without "
                "compaction.",
                self.name,
            )
    ...
    def _create_toolkit(
        ...
        tool_functions: dict[str, Any] = {
            ...
            "create_agentdesk_employee": create_agentdesk_employee,
            "create_agentdesk_employees": create_agentdesk_employees,
            **(
    ...
    def _auto_continue_hint_count(self) -> int:
        """Count auto-continue hints since the last real user turn."""
        count = 0
        for msg in reversed(getattr(self.state, "context", []) or []):
            if getattr(msg, "role", None) != "user":
                continue
            raw = msg.get_text_content() if msg is not None else ""
            text = (raw or "").strip()
            if not text.startswith("<system-hint>"):
                break
            if (
                "Your previous assistant turn had text only" in text
                or "上轮助手仅文字" in text
            ):
                count += 1
                continue
    ...
    _TASK_COMPLETION_RE = re.compile(
        ...
        r"|all\s+(steps?\s+)?(done|complete|finished)"
        r"|waiting\s+for\s+(your\s+)?(next\s+)?(task|question|message))",
        re.IGNORECASE,
    )

    _PLANNING_CONTINUE_RE = re.compile(
        r"(接下来"
        r"|我将"
        r"|我会"
        r"|首先需要"
    ...
    _CASUAL_USER_RE = re.compile(
        r"^(你好|您好|hi|hello|hey|在吗|在不在|谢谢|感谢|好的|嗯|哦|啊|哈哈|呵呵|"
        r"你是谁|你能做什么)[\s!！?？。．.…,，~～]*$",
        re.IGNORECASE,
    )

    @classmethod
    def _looks_like_in_progress_planning(cls, msg: Msg | None) -> bool:
        """True when text-only output signals tools still needed."""
        if msg is None:
            return False
        text = (msg.get_text_content() or "").strip()
        if not text:
            return False
        return cls._PLANNING_CONTINUE_RE.search(text) is not None

    def _last_real_user_text(self) -> str:
        """Most recent user turn, skipping auto-continue system hints."""
        for msg in reversed(getattr(self.state, "context", []) or []):
            if getattr(msg, "role", None) != "user":
                continue
            raw = msg.get_text_content() if msg is not None else ""
            text = (raw or "").strip()
            if not text or text.startswith("<system-hint>"):
                continue
            return text
        return ""

    def _is_casual_user_turn(self) -> bool:
        text = self._last_real_user_text().strip()
        if not text or len(text) > 80:
            return False
        return self._CASUAL_USER_RE.match(text) is not None

    @classmethod
    def _looks_like_task_completion(cls, msg: Msg | None) -> bool:
        """True when a text-only assistant turn is clearly wrapping up."""
        if msg is None:
            return False
        text = (msg.get_text_content() or "").strip()
        if not text or len(text) > 1200:
            return False
        return cls._TASK_COMPLETION_RE.search(text) is not None
    ...
    def _should_auto_continue(
        ...
        if self._looks_like_task_completion(msg):
            return False

        if self.state.cur_iter >= self.react_config.max_iters - 1:
            return False

        if self._auto_continue_hint_count() >= self._AUTO_CONTINUE_MAX_EXTRA:
            return False

        if self._is_casual_user_turn():
            return False

        return self._looks_like_in_progress_planning(msg)
