...
_GENERIC_HUB_NAMES = frozenset({"skill", "skills"})


def _is_generic_hub_name(name: str) -> bool:
    return not name or name.strip().lower() in _GENERIC_HUB_NAMES


def _slug_from_bundle_url(bundle_url: str) -> str:
    return urlparse(bundle_url).path.strip("/").split("/")[-1]


def _title_from_hub_summary(summary: str) -> str:
    text = summary.strip()
    if not text:
        return ""
    for sep in (" — ", " – ", " - "):
    ...
    return truncated.strip() or text[:80].strip()


def _humanize_hub_slug(slug: str) -> str:
    base = slug.strip().removesuffix("-skill").removesuffix("_skill")
...
def _hub_search_display_name(item: dict[str, Any], slug: str) -> str:
    for key in ("name", "title"):
        value = str(item.get(key) or "").strip()
        if value and not _is_generic_hub_name(value):
            return value
    display = str(item.get("displayName") or "").strip()
    if display and not _is_generic_hub_name(display):
        return display
    summary = str(
        item.get("summary") or item.get("description") or "",
    ).strip()
    if summary:
        title = _title_from_hub_summary(summary)
        if title:
            return title
    return _humanize_hub_slug(slug)


def _resolve_hub_install_name(
    *,
    bundle_name: str,
    bundle_url: str,
    target_name: str | None,
) -> str:
    slug_fallback = _safe_fallback_name(_slug_from_bundle_url(bundle_url))
    name = bundle_name.strip()
    if _is_generic_hub_name(name):
        name = slug_fallback
    normalized_target = str(target_name or "").strip()
    if normalized_target and not _is_generic_hub_name(normalized_target):
        return _sanitize_skill_dir_name(normalized_target)
    return _sanitize_skill_dir_name(name or slug_fallback)
...
async def search_hub_skills(
    ...
    for item in items:
        ...
        if not source_url:
            source_url = _join_url(base, slug)
        results.append(
            HubSkillResult(
                slug=slug,
                name=_hub_search_display_name(item, slug),
                description=str(
...
async def _prepare_install_payload(
    ...
    name = _resolve_hub_install_name(
        bundle_name=name,
