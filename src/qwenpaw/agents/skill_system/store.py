...
def render_skill_md(
    *,
    ...
    from .skill_icons import derive_skill_icon_key
    ...
    post["metadata"] = {
        "qwenpaw": {
            "icon": derive_skill_icon_key(proposed_name, description),
        },
    }
...
def _extract_qwenpaw_metadata(metadata: Any) -> dict[str, Any]:
    if not isinstance(metadata, dict):
        return {}
    qwenpaw = metadata.get("qwenpaw")
    return qwenpaw if isinstance(qwenpaw, dict) else {}


def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract legacy emoji from metadata.qwenpaw.emoji."""
    return str(_extract_qwenpaw_metadata(metadata).get("emoji", "") or "")


def _extract_icon_from_metadata(metadata: Any) -> str:
    """Extract tool icon key from metadata.qwenpaw.icon."""
    from .skill_icons import is_skill_icon_key

    icon = str(_extract_qwenpaw_metadata(metadata).get("icon", "") or "").strip()
    return icon if is_skill_icon_key(icon) else ""
...
def read_skill_from_dir(skill_dir: Path, source: str) -> SkillInfo | None:
    ...
    try:
        ...
        try:
            ...
            emoji = _extract_emoji_from_metadata(meta)
            icon = _extract_icon_from_metadata(meta)
        except Exception:
            pass
        ...
        if not icon:
            ...
            icon = derive_skill_icon_key(skill_dir.name, description)
