...
class BootstrapHook:
    ...
    async def __call__(
        ...
        try:
            ...
            team_leader_meta = self.working_dir / "TEAM_LEADER.json"
            employee_meta = self.working_dir / "EMPLOYEE.json"

            # Pre-configured AgentDesk roles skip generic BOOTSTRAP onboarding.
            if team_leader_meta.exists() or employee_meta.exists():
                return None
