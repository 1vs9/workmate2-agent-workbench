...
class ConsoleConfig(BaseChannelConfig):
    ...
    enabled: bool = True
    media_dir: Optional[str] = None
    # Reasoning/thinking blocks are verbose; hide them from the backend
    # terminal by default (still streamed to AgentDesk UI via SSE).
...
def _default_builtin_tools() -> Dict[str, BuiltinToolConfig]:
    ...
    tools = {
        ...
        "create_agentdesk_employee": BuiltinToolConfig(
            name="create_agentdesk_employee",
            enabled=True,
            description=(
                "Create a AgentDesk digital employee via plaza + join APIs"
            ),
            icon="🪪",
        ),
    }
