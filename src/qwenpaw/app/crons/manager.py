...
class CronManager:
    ...
    async def _dream_callback(self) -> None:
        ...
        try:
            # Non-default AgentDesk agents may intentionally have no
            # long-term memory backend.
            memory_manager = getattr(self._runner, "memory_manager", None)
            if memory_manager is None:
                logger.debug("Dream task skipped: memory manager unavailable")
                return
            # Run dream task
            await memory_manager.dream()
