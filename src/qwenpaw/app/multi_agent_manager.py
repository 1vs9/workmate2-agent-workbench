...
class MultiAgentManager:
        ...
        if not should_start:
            try:
                ...
                raise ConfigurationException(
                    config_key="agent",
                    message=(
                        f"Agent '{agent_id}' startup timed out "
                        f"after 30 seconds"
                    ),
                )
            if agent_id in self.agents:
    ...
    async def _graceful_stop_old_instance(
        ...
        if has_active:
            ...
            async def delayed_cleanup():
                ...
                async def _body():
                    completed = await (
                        old_instance.task_tracker.wait_all_done(
                            timeout=60.0,
                        )
                    )
                    if completed:
                ...
                try:
                    await asyncio.wait_for(_body(), timeout=300.0)
                except asyncio.TimeoutError:
                    logger.warning(
                        f"Total timeout (300s) reached for delayed "
                        f"cleanup of {agent_id}. Abandoning old instance.",
                    )
                except Exception as e:
                    logger.warning(
                        f"Error during delayed cleanup for {agent_id}: "
                        f"{e}. New instance is serving requests.",
                    )
    ...
    async def stop_agent(self, agent_id: str) -> bool:
        ...
        async with self._lock:
            ...
            instance = self.agents.pop(agent_id)
    ...
    async def reload_agent(self, agent_id: str) -> bool:
        ...
        if event is not None:
            # Agent currently being lazy-loaded; wait for it
            await event.wait()
            async with self._lock:
                if agent_id not in self.agents:
                    return False
        ...
        cached_old_instance = old_instance
        ...
        if cached_old_instance:
            # Get all reusable services from old instance's ServiceManager
            # pylint: disable=protected-access
            reusable = (
                cached_old_instance._service_manager.get_reusable_services()
            )
            # pylint: enable=protected-access
        ...
        await self._graceful_stop_old_instance(cached_old_instance, agent_id)
