...
def schedule_agent_reload(request: "Request", agent_id: str) -> None:
    ...
    try:
        asyncio.get_running_loop().create_task(reload_in_background())
    except RuntimeError:
        # Sync endpoints run in a worker thread without a running loop.
        main_loop = getattr(request.app.state, "loop", None)
        if main_loop is None or not main_loop.is_running():
            logger.warning(
                "Cannot schedule agent reload for '%s': no running event loop",
                agent_id,
            )
            return
        main_loop.call_soon_threadsafe(
            lambda: main_loop.create_task(reload_in_background()),
        )
