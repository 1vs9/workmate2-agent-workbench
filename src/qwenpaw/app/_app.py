...
async def _warm_team_leader_agents(multi_agent_manager) -> None:
    """Background-warm hidden team-leader agents to remove first-message cold start.

    Loads each ``lead_*`` agent workspace once, throttled, so the first team
    chat after a restart streams immediately instead of paying a multi-second
    workspace-init cost. Failures are non-fatal: any agent that fails here just
    lazy-loads on first use as before.
    """
    try:
        from ..agentdesk.team_leader_agents import is_team_leader_agent_id

        config = load_config(get_config_path())
        leader_ids = [
            agent_id
            for agent_id, ref in config.agents.profiles.items()
            if getattr(ref, "enabled", True) and is_team_leader_agent_id(agent_id)
        ]
    except Exception:
        logger.debug("Skipped team-leader warm pool (config unavailable)", exc_info=True)
        return

    for agent_id in leader_ids:
        if agent_id in multi_agent_manager.agents:
            continue
        try:
            await multi_agent_manager.get_agent(agent_id)
            logger.debug("Warmed team-leader agent %s", agent_id)
        except Exception:
            logger.debug(
                "Failed to warm team-leader agent %s; will lazy-load on first use",
                agent_id,
                exc_info=True,
            )
        # Yield between loads so warming never monopolizes the event loop.
...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    ensure_qa_agent_exists()
    try:
        from ..agentdesk.builtin_agents import maybe_seed_builtin_agents
        from ..agentdesk.default_agent import ensure_agentdesk_default_agent_identity
        ...
        if is_agentdesk_enabled():
            ...
            def _seed_builtin_agents() -> None:
                try:
                    ...
                    logger.exception("AgentDesk builtin agent seed failed")

            threading.Thread(
                target=_seed_builtin_agents,
                daemon=True,
                name="agentdesk-builtin-seed",
            ).start()
    except ImportError:
        pass
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            # Start configured agents. In AgentDesk mode only the ``default``
            # agent carries long-term memory and must be warm for an instant
            # first reply; position agents (``emp_*``) and team leaders
            # (``lead_*``) are session-scoped with no long-term memory, so we
            # let MultiAgentManager.get_agent() lazy-load them on first use
            # instead of eagerly warming every profile at boot (which is a main
            # cause of slow startup once many employees/teams exist). Non-AgentDesk
            # behavior is unchanged.
            try:
                from ..agentdesk.settings import is_agentdesk_enabled
            except Exception:
                is_agentdesk_enabled = None  # type: ignore[assignment]
            if is_agentdesk_enabled is not None and is_agentdesk_enabled():
                from ..agentdesk.default_agent import DEFAULT_AGENT_ID

                default_agent_id = str(DEFAULT_AGENT_ID or "").strip()
                if default_agent_id:
                    try:
                        await multi_agent_manager.get_agent(default_agent_id)
                    except Exception:
                        logger.warning(
                            "Failed to warm default agent %s at startup; "
                            "it will lazy-load on first request",
                            default_agent_id,
                            exc_info=True,
                        )

                # Warm team-leader agents (``lead_*``) in the background. There is
                # no idle eviction, so an agent is only cold on its first message
                # of the process; warming the (few) team leaders here removes the
                # multi-second cold-start the user feels on the first team chat
                # after a restart. This runs as its own task so it never delays
                # the rest of background init, and is throttled to avoid a CPU
                # spike. Employees (``emp_*``) stay lazy: there can be many and
                # they load on delegation anyway.
                asyncio.create_task(
                    _warm_team_leader_agents(multi_agent_manager),
                )
            else:
...
def read_root():
    try:
        from ..agentdesk.settings import (
            get_frontend_dir,
            get_next_frontend_dir,
            is_agentdesk_enabled,
        )

        if is_agentdesk_enabled():
            frontend_dir = get_next_frontend_dir() or get_frontend_dir()
...
_agentdesk_mode = False
_agentdesk_frontend_mounted = False
try:
    from ..agentdesk.settings import is_agentdesk_enabled
    from ..agentdesk.frontend import mount_agentdesk_frontend
    from ..agentdesk.chat import router as agentdesk_chat_router
    from ..agentdesk.router import api_router as agentdesk_api_router
    from ..agentdesk.router import router as agentdesk_router

    _agentdesk_mode = is_agentdesk_enabled()
    if _agentdesk_mode:
        app.include_router(agentdesk_router)
        app.include_router(agentdesk_api_router)
        app.include_router(agentdesk_chat_router)
        logger.info("AgentDesk compatibility layer enabled")
except ImportError:
    logger.debug("AgentDesk package not available", exc_info=True)
...
# AgentDesk frontend (demo-plat) — replaces QwenPaw console SPA when enabled.
if _agentdesk_mode:
    _agentdesk_frontend_mounted = mount_agentdesk_frontend(app)

# Console static files and SPA fallback
# Register these AFTER API routes to ensure proper routing priority
if not _agentdesk_frontend_mounted and os.path.isdir(_CONSOLE_STATIC_DIR):
