...
class Workspace:
    ...
    def _register_services(  # pylint: disable=too-many-statements
        ...
        should_register_memory_manager = True
        try:
            from ...agentdesk.settings import is_agentdesk_enabled
        except Exception:  # pragma: no cover - AgentDesk optional import
            is_agentdesk_enabled = None
        if is_agentdesk_enabled is not None and is_agentdesk_enabled():
            ...
            except Exception:  # pragma: no cover - keep native fallback
                default_agent_id = ""
            if default_agent_id:
                should_register_memory_manager = (
                    str(self.agent_id).strip() == default_agent_id
                )
        ...
        if should_register_memory_manager:
            sm.register(
                ServiceDescriptor(
                    name="memory_manager",
