...
def session_state_to_agent_messages(state: dict) -> List[Msg]:
    """Read AgentScope messages from a saved session state dict.

    The canonical 2.0 path stores AgentScope messages under
    ``agent.state.context``. 1.x sessions may still store ``agent.memory``;
    keep that compatibility here so all chat-history readers share the same
    migration behavior.
    """
...
def session_state_to_messages(state: dict) -> List[Message]:
    """Convert a saved session state dict into runtime messages."""

    memories = session_state_to_agent_messages(state)
    if not memories:
        return []
