...
from qwenpaw.agents.tools.agent_management import worker_stream_publish_key
...
from ..chat_task_store import (
    DEFAULT_STREAM_TASK_TIMEOUT,
    build_task_allowed_readers,
    chat_task_store,
    run_background_console_chat,
)
...
def _resolve_request_agent_id(request: Request) -> str:
    """Best-effort agent id for the current HTTP request."""
    state_agent = getattr(request.state, "agent_id", None)
    if state_agent:
        return str(state_agent).strip()
    header_agent = request.headers.get("X-Agent-Id")
    if header_agent:
        return str(header_agent).strip()
    return ""


def _normalize_task_timeout(raw_timeout: Any) -> float:
    """Coerce client timeout to a finite positive seconds value."""
    try:
    ...
    if not (0 < value < float("inf")):
        raise HTTPException(
            status_code=400,
            detail="timeout must be a finite positive number",
        )
    return value
...
@router.post(
    ...
    description="Returns immediately with a task_id. Poll GET /console/chat/task/{task_id} "
    "or use the check_agent_task tool for status.",
)
async def post_console_chat_task(
    request_data: Union[AgentRequest, dict],
    request: Request,
) -> dict:
    """Submit a chat run that executes asynchronously."""
    workspace = await get_agent_for_request(request)
    console_channel = await workspace.channel_manager.get_channel("console")
    if console_channel is None:
        raise HTTPException(
    ...
    task_timeout = DEFAULT_STREAM_TASK_TIMEOUT
    publish_key: str | None = None
    if isinstance(request_data, dict):
        raw_timeout = request_data.get("timeout")
        if raw_timeout is not None:
            task_timeout = _normalize_task_timeout(raw_timeout)
    meta = native_payload.get("meta") if isinstance(native_payload.get("meta"), dict) else {}
    session_id = str(meta.get("session_id") or "").strip()
    if not session_id and isinstance(request_data, dict):
        session_id = str(request_data.get("session_id") or "").strip()
    root_session_id: str | None = None
    if isinstance(request_data, dict):
        raw_root = request_data.get("root_session_id")
        if isinstance(raw_root, str) and raw_root.strip():
            root_session_id = raw_root.strip()
        elif isinstance(request_data.get("request_context"), dict):
            nested = request_data["request_context"].get("root_session_id")
            if isinstance(nested, str) and nested.strip():
                root_session_id = nested.strip()
    publish_key = worker_stream_publish_key(session_id, root_session_id)

    executor_agent_id = _resolve_request_agent_id(request)
    allowed_readers = build_task_allowed_readers(
        executor_agent_id=executor_agent_id,
        request_data=request_data if isinstance(request_data, dict) else None,
    )
    record = await chat_task_store.create(
        executor_agent_id=executor_agent_id,
        allowed_reader_agent_ids=allowed_readers,
    )
    asyncio.create_task(
        run_background_console_chat(
            ...
            to_agent=str(executor_agent_id),
            publish_key=publish_key,
            task_timeout=task_timeout,
        ),
    )
    return chat_task_store.to_status_payload(record)
...
async def get_console_chat_task(
    ...
    """Return lifecycle status and, when finished, the agent response payload."""
    await get_agent_for_request(request)
    record = await chat_task_store.get(task_id)
    if record is None:
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    reader_agent_id = _resolve_request_agent_id(request)
    if not record.is_readable_by(reader_agent_id):
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    return chat_task_store.to_status_payload(record)
