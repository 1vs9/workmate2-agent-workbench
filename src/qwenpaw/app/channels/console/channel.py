...
class ConsoleChannel(BaseChannel):
    ...
    @staticmethod
    def _ev_type_label(ev_type: Any) -> str:
        if ev_type is None:
            return ""
        if hasattr(ev_type, "value"):
            return str(ev_type.value)
        return str(ev_type)
    ...
    def _print_completed_message(
        self,
        event: Any,
        *,
        reasoning_printed: set[str],
    ) -> None:
        """Pretty-print one completed assistant message to stdout."""
        ev_type = getattr(event, "type", None)
        msg_id = str(getattr(event, "id", None) or "")

        if ev_type == MessageType.REASONING:
            if msg_id and msg_id in reasoning_printed:
                return
            if msg_id:
                reasoning_printed.add(msg_id)

            body = self._message_plain_text(event)
            char_len = len(body)
            if self._filter_thinking:
                logger.debug(
                    "console reasoning complete (%s chars, msg_id=%s)",
                    char_len,
                    msg_id or "?",
                )
                return
            ...
            ts = _ts()
            self._safe_print(
                f"{_YELLOW}💭 [{ts}] Reasoning complete "
                f"({char_len} chars){_RESET}",
            )
            if body:
                logger.debug("console reasoning body: %s", body)
            return

        parts = self._message_to_content_parts(event)
        if not parts:
            return
        self._print_parts(parts, self._ev_type_label(ev_type))
        ...
        try:
            ...
            async for event in self._process(request):
                ...
                if (
                    ...
                    msg_id = str(getattr(event, "id", None) or "")
                    if msg_id and msg_id not in reasoning_started:
                        reasoning_started.add(msg_id)
                        logger.debug(
                            "console reasoning started (msg_id=%s)",
                            msg_id,
                        )
