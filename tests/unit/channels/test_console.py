...
class TestConsoleStreaming:
    ...
    async def test_stream_one_skips_reasoning_stdout_when_filter_thinking(
        self,
        stream_channel,
        capsys,
    ):
        """Completed reasoning blocks should not flood stdout when filtered."""
        from qwenpaw.schemas import (
            Message,
            MessageType,
            Role,
        ...
        stream_channel._filter_thinking = True
        ...
        async for _ in stream_channel.stream_one(payload):
            pass

        captured = capsys.readouterr()
        assert "Reasoning" not in captured.out
        assert "拆解任务" not in captured.out
        assert "Bot" not in captured.out

    async def test_stream_one_reasoning_compact_line_when_not_filtered(
        self,
        stream_channel,
        capsys,
    ):
        """When filter_thinking is off, print one summary line, not full body."""
        from qwenpaw.schemas import (
            Message,
            MessageType,
            Role,
        ...
        stream_channel._filter_thinking = False
        long_body = "x" * 500
        ...
        reasoning = Message(
            ...
            role=Role.ASSISTANT,
            status=RunStatus.Completed,
            content=[TextContent(type=ContentType.TEXT, text=long_body)],
        )
        ...
        captured = capsys.readouterr()
        assert "Reasoning complete (500 chars)" in captured.out
