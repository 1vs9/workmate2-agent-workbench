...
async def test_create_agent_appends_new_id_to_order(monkeypatch, tmp_path):
    ...
    def fake_provision_agent_profile(**kwargs):
        ...
        config.agents.profiles["beta"] = AgentProfileRef(
            id="beta",
            workspace_dir=str(tmp_path / "beta"),
            enabled=True,
        )
        config.agents.agent_order = ["alpha", "default", "beta"]
        return config.agents.profiles["beta"]
    ...
    assert captured["workspace_dir"] == str(tmp_path / "beta")
