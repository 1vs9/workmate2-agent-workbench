...
from qwenpaw.agents import provisioning as agent_provisioning
...
def test_initialize_agent_workspace_creates_parent_dirs(
    monkeypatch,
    tmp_path,
):
    """Nested workspace paths should be created when parents are missing."""
    import qwenpaw.config as config_module

    nested = tmp_path / "workspaces" / "lead_test"
    monkeypatch.setattr(
        config_module,
        "load_config",
        lambda: _stub_global_config("en"),
    ...
    agents_router._initialize_agent_workspace(nested)

    assert nested.is_dir()
    assert (nested / "sessions").is_dir()
    assert (nested / "memory").is_dir()
