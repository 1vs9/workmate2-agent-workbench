...
class TestBootstrapHookCallEarlyExit:
    ...
    async def test_returns_none_for_team_leader_workspace(
        self,
        hook,
        mock_agent,
    ):
        """Team leader orchestrators skip bootstrap even if BOOTSTRAP.md exists."""
        (hook.working_dir / "BOOTSTRAP.md").write_text("# Bootstrap")
        (hook.working_dir / "TEAM_LEADER.json").write_text(
            '{"is_team_leader":true}',
            encoding="utf-8",
        )
        result = await hook(mock_agent, {})
    ...
    async def test_returns_none_for_employee_workspace(
        self,
        hook,
        mock_agent,
    ):
        """AgentDesk employees skip bootstrap even if BOOTSTRAP.md exists."""
        (hook.working_dir / "BOOTSTRAP.md").write_text("# Bootstrap")
        (hook.working_dir / "EMPLOYEE.json").write_text(
            '{"is_agentdesk_employee":true,"name":"代码质量守护者"}',
            encoding="utf-8",
        )
        result = await hook(mock_agent, {})
        assert result is None
